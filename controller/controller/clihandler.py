...
class Command(object):
    ...
    def sanity(self):
        ...
        if 'domainid' not in opts:
            entry = Domain.getone()
            opts['domainid'] = entry.domainid
        if 'envid' not in opts:
        ...
        #
        if 'uuid' in opts and 'displayname' not in opts \
                and 'hostname' not in opts and 'type' not in opts:
            pass
        elif 'uuid' not in opts and 'displayname' not in opts \
                and 'hostname' not in opts and 'type' not in opts:
...
#            sys.stdout.write(line)
    ...
    def do_status(self, cmd):
        ...
        self.ack()
        body = self.server.cli_cmd(
            "tabadmin status -v", agent, timeout=60 * 30)
    ...
    def do_backup(self, cmd):
        ...
        self.server.event_control.gen(
            backup_started_event, data, userid=userid)
        self.ack()
    ...
    def do_restore(self, cmd):
        ...
        if 'nobackup' in cmd.dict:
            backup_first = False
        else:
            backup_first = True

        if 'nolicense' in cmd.dict:
            license_check = False
        else:
            license_check = True
        ...
        if backup_first:
            ...
            if success(body):
                ...
                self.server.event_control.gen(real_event, dict(
                    body.items() + data.items()), userid=userid)
    ...
    def do_restore_url(self, cmd):
        ...
        if url.scheme not in ('file', 's3', 'gs'):
    ...
    def do_license(self, cmd):
        ...
        if len(cmd.args) == 1:
            action = cmd.args[0].lower()
            if action not in ['repair', 'info', 'verify', 'send', 'update']:
                self.print_usage(self.do_license.__usage__)
                return
    ...
    def do_daily(self, cmd):
        ...
        entry = self.server.system.entry(
            SystemKeys.EMAIL_SPIKE_DISABLED_ALERTS)
...
#          (agent.displayname, agent.agent_type))
    ...
    def _do_cloud_test(self, cloud_type, cmd):
        ...
        if option_count:
            if option_count != 3:
                self.error(
                    clierror.ERROR_COMMAND_SYNTAX_ERROR,
    ...
    def do_support_case(self, cmd):
        ...
        body = self.server.support_case(
            agent, userid=userid, filename=filename)
        agent.connection.user_action_unlock()
        return self.report_status(body)
    ...
    def get_agent(self, opts, error_on_no_agent=True):
        ...
        if 'uuid' in opts:  # should never fail
    ...
    def handle(self):
        while True:
            try:
                ...
                self.error(
                    clierror.ERROR_SOCKET_DISCONNECTED,
            ...
            # fixme on exceptions: reset state?
                ...
                raise
            except exc.InvalidStateError as ex:
