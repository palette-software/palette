...
class Application(GenericWSGIApplication):
    ...
    def post(self, req):
        ...
        main_sasl_cf_fname = os.path.join(
            exec_dir, parser.get(
                'postfix', 'main_sasl_cf'))
    ...
    def getdata(self, req):
        ...
        if 'alert_email_address' not in data:
        ...
        if data['mail_server_type'] == 2:
            if 'smtp_server' not in data or not data['smtp_server']:
                raise PostfixException("Missing mail server for relay")

            if 'smtp_port' not in data or not data['smtp_port']:
            ...
            if 'smtp_username' in data and 'smtp_password' not in data:
                raise PostfixException(
